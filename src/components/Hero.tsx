...
const Hero = ({ 
  ...
  return (
    <section 
      className="relative bg-gradient-to-br from-blue-600 via-blue-700 to-green-600 text-white overflow-hidden"
      role="banner"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <div className="order-1 lg:order-2 w-full flex justify-center lg:justify-end">
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl px-4 sm:px-6 xl:px-8 py-6 xl:py-8 border border-white/20 w-full max-w-md">
              ...
              <nav role="navigation" aria-label="Service selection">
                ...
                >
                  <Link 
                    to="/eco4"
                    aria-describedby="eco4-description"
                  >
                    ECO4
                  </Link>
                </Button>
                <div id="eco4-description" className="sr-only">
                  Navigate to ECO4 energy efficiency scheme information and application
                </div>
                <Button 
                  asChild 
                  className="w-full justify-center p-3 xl:p-4 bg-white/10 hover:bg-white/20 text-white hover:text-white hover:font-bold border-white/20 transition-all duration-300 hover:shadow-[0_0_20px_rgba(255,255,255,0.5)] hover:scale-105 touch-manipulation min-h-[48px]"
          ...
          <div className="order-2 lg:order-1">
            ...
            {benefits.length > 0 && (
              <div className="mb-6 sm:mb-8">
                <ul className="space-y-2 sm:space-y-3" role="list" aria-label="Key benefits">
                  {benefits.map((benefit, index) => (
                    <li key={index} className="flex items-start space-x-3">
                      <CheckCircle className="h-5 w-5 sm:h-6 sm:w-6 text-green-300 flex-shrink-0 mt-0.5 sm:mt-0" aria-hidden="true" />
